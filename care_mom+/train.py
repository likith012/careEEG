...
def Pretext(
    ...
    for epoch in range(Epoch):
        ...
        for index, (anc, pos) in enumerate(
            ...
            # backprop
            epoch_len = top_res.shape[1] # 7
            
            loss1 = 0
            for i in range(epoch_len):
                loss1 += criterion(top_res[:, i, :], bot_res[:, i, :])
            
            loss2 = criterion(top_tfr, bot_tfr) 
            
            loss3 = 0
            for i in range(epoch_len):
                loss3 += criterion(top_res[:, i, :], bot_tfr)
            
            loss4 = 0
            for i in range(epoch_len):
                loss4 += criterion(top_tfr, bot_res[:, i, :])
            
            loss = (loss1.mean() + loss2) + lambda1 * (loss3.mean() + loss4.mean())
